  ...
  <div v-loading="loading" element-loading-text="拼命运算中">
    <el-container>
      <el-tabs id="modelList"  v-model="model" tab-position="left">
        ...
        <el-tab-pane v-for="(tab, index) in tabPages" :key="index" :label="tab.algorithmName" @tab-click="onTabClick">
            ...
            >
              <el-form-item prop="K_OR">
                <template slot="label">
                  ...
                  <el-popover placement="right" trigger="hover">
                    <div>
                      根据需求选择不同的任务类型挖掘数据,空间的层次分为临床表征指标、社会环境指标、生活行为习惯指标
                    </div>
                    <el-icon
                      slot="reference"
              ...
              <el-form-item prop="K_and_SP">
                ...
                <el-select v-model="selectedAlgorithm" style="margin-top: 5px;" placeholder="请选择算法">
                  <el-option v-for="(tab, index) in tabPages"  v-model="tab.algorithmName" :key="index" :label="tab.algorithmName"   :value="tab.algorithmName">
                              </el-option>
...
mounted(){
  ...
  methods: {
    onTabClick(tab) {
      console.log(tab)
      this.model = tab.name; // 更新model
      // 获取当前选中的tabPages项
      const currentTab = this.tabPages.find(t => t.algorithmName === tab.name);
      if (currentTab) {
        this.ll = tab.algorithmName; // 将当前标签页的算法名称赋给ll
...
searchAll () {
 ...
 //提交整个任务
      ...
      //this.from.dataType=this.all.m_dataType;
