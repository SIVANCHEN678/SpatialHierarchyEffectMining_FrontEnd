<template>
  <div class="Box">
    <div style="text-align: center;margin-top: 10px;background: #ffd9e6;border-radius: 50px">
          <span style="font-weight: bold">特征下的进度条为该特征的缺失率</span>
        </div>
    <!-- ------------------------------------选择目标特征 ------------------------------->
...
export default {
  ...
  handleCheckAllChange(val) {
    ...
    init() {
      this.table1=this.$store.state.disFactor.m_disease
      this.disease=this.$store.state.disFactor.m_dataset//根据表名找到所有的变量名，然后在疾病里面找是否有这个特征
