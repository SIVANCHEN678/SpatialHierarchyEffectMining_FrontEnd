      ...
      <div v-if="item.columnisR" class="checkbox-container">
        <el-tooltip effect="light" :content="item.columnDesc" placement="top">
          <el-checkbox class="radio" v-model="checkedDemoOptions" :label="item.columnName" border>
            {{ item.columnName }}
            <el-progress v-if="featuresMissingRate[item.columnName]" :percentage="featuresMissingRate[item.columnName]"
                            :format="format"
          ...
          <el-checkbox class="radio" :label="item.columnName" border>
                <el-radio v-for="feature in sociolOptions" :key="feature" :label="feature" style="display: inline-block;" >
                  <span style="width: 260px; height: 20px; display: inline-block; line-height: 20px;">{{ feature.columnName }}</span>
                  <el-progress :percentage="featuresMissingRate[ feature.columnName ]"
                               :format="format"
                               :color="getProgressStatus(featuresMissingRate[ feature.columnName ])"
