...
export default {
  ...
  methods: {
    ...
    city(){
      ...
      const containsDataset = this.showDataForm.tablename.includes("公共数据集");
console.log(containsDataset); // 如果包含则返回true，否则返回false
    ...
    //   }
      this.m_changeTaskInfo({ m_disease: this.dise});//设置病种
      this.m_changeTaskInfo({ m_dataset: this.chosenData });//直接设置
      // this.m_changeTaskInfo({ m_disease: '高血压'});
    ...
    changeData(data){
      ...
      getRequest(`admin/api/getDiseaseName/${this.table1}`).then(
                    (response) => {
