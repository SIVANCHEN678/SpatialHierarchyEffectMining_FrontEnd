...
searchAll () {
  ...
  getRequest('admin/algorithm/getAll', {}).then(
    (response) => {
      ...
      // 设置默认激活的tab页为名称为'pc'且description为'123'的那个
      const defaultTabPage = response.find(tab => tab.algorithmName === 'fges' && tab.algorithmDescription === '123');
      if (defaultTabPage) {
        this.activeTab = defaultTabPage.algorithmName; // 或者使用其他能唯一标识tab页的字段
      } else {
        // 如果后台数据中没有匹配项，根据需要设定一个默认值
        this.activeTab = response[0].algorithmName; // 或者任何你想默认展示的tab页名称
      }
    }
  );
},
