    ...
    <el-col v-for="item in demoOptions" :key="item.columnName" :span="8">
      <div v-if="item.columnisR" class="checkbox-container">
        <el-tooltip effect="light" :content="item.columnDesc" placement="top">
          <el-checkbox class="radio" v-model="checkedDemoOptions"  :checked="isChecked(demoOptions, item)" 
            @change="toggleChecked('demoOptions', item)" :label="item.columnName"  border>
            {{ item.columnName }}
            <el-progress :percentage="featuresMissingRate[item.columnName]"
                            :format="format"
                            v-if="featuresMissingRate[item.columnName]"
          ...
          <el-col v-for="item in sociolOptions" :key="item.columnName" :span="8">
            <div v-if="item.columnisX" class="checkbox-container">
              <el-tooltip effect="light" :content="item.columnDesc" placement="top">
                <el-checkbox class="radio" :label="item.columnName" :checked="isChecked(sociolOptions, item)" 
            @change="toggleChecked('sociolOptions', item)" border>
                  {{ item.columnName }}
                  <el-progress  v-if="featuresMissingRate[item.columnName]"
                             :percentage="featuresMissingRate[item.columnName]"
                            :format="format"
                            :color="getProgressStatus(featuresMissingRate[item.columnName])"
                            style="width: 100px;">
          </el-progress>
          ...
          <!-- 生活行为习惯指标 -->
          <el-row :gutter="20">
            <h5 class="text" style="font-size: 20px; margin-left: 0.5%; font-weight: bold;"> <span class="lineStyle">▍</span>生活行为习惯指标</h5>
            <el-col v-for="item in physioOptions" :key="item.columnName" :span="8">
              <div v-if="item.columnisS" class="checkbox-container">
                <el-tooltip effect="light" :content="item.columnDesc" placement="top">
                  <el-checkbox class="radio"  :checked="isChecked(checked_dependent_variables, item)" 
            @change="toggleChecked(item)" :label="item.columnName" border>
                    {{ item.columnName }}
                    <el-progress :percentage="featuresMissingRate[item.columnName]"
                            :format="format"
                            v-if="featuresMissingRate[item.columnName]"
        ...
        <div class="select_feature_var">
          <div class="select_feature_var_top">
            <h5 class="text" style="font-size: 20px; font-weight: bold;"> <span class="lineStyle">▍</span>已选特征</h5>
           
          </div>
          <div class="select_feature_check_boxs">
            <el-checkbox-group v-model="checked_dependent_variables">
              <div v-for="item in this.columnSelectForm.formData.selectedData" :key="item.id">
...
export default {
  ...
  methods: {
    isChecked(optionList, item) {
    return optionList.some(opt => opt.columnName === item.columnName && this['checked' + optionList.constructor.name] ?.includes(item));
  },
  toggleChecked(listName, item) {
    const checkedList = this['checked' + listName];
    const index = checkedList.findIndex(opt => opt.columnName === item.columnName);
