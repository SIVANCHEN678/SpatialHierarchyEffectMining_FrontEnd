...
export default {
  ...
  methods: {
    ...
    // 将选定的item添加或移除到已选特征列表中
    if (item.isChecked) {
      this.checked_dependent_variables.push(item);
    } else {
      const index = this.checked_dependent_variables.findIndex(i => i.columnName === item.columnName);
      if (index !== -1) {
        this.checked_dependent_variables.splice(index, 1);
      }
    }
  },
