<template>
  <div class="main"> 
    <div class="left_tree">
      ...
      <span class="lineStyle" style="margin-left: 20%;margin-bottom: 5%;"></span><span><b>查询历史任务</b></span>
      ...
      <div id="top_buttons">
        <div id="task_leader">
         ...
         <span> 任务名称：</span>  <el-input type="text" v-model="searchText" @keyup.enter="onSearch" :style="{ width: '200px' }" placeholder="请输入查询内容..."></el-input>
        </div>
        <el-button @click="clearFilter">清除</el-button>
        <!-- <el-button type="success" @click="showSelect = true">新建任务</el-button> -->
    
    <el-dialog title="新建任务" :visible.sync="showSelect" width="30%">
      <div>
        <el-radio-group v-model="selectedOption" @change="goToRoute">
          <el-radio :label="1">选项1（跳转到路由A）</el-radio>
          <el-radio :label="2">选项2（跳转到路由B）</el-radio>
        </el-radio-group>
...
async onSearch() {
      ...
      const queryResult = await this.fetchData(this.searchText);
    ...
    clearFilter() {
      this.leader='',
      this.AI='',
      this.searchObj.disease = "";
     this.searchObj.algorithmId = "";
     this.searchObj.leader = "";
     this.getList();
    },
    run(row) {
...
_this.$router.push({ path: '/she/result' ,query: { form: row}})
    ...
    postRequest(`admin/mining/findMining/${this.current}/${this.limit}`, this.searchObj)
      .then(response => { //请求成功response是接口返回数据
        ...
        //this.taskLeaderList=list.leader;
