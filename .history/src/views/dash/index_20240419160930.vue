<template>
  <el-row :gutter="40" class="panel-group">
    ...
    <el-col :xs="12" :sm="12" :lg="6" class="card-panel-col">
      <span class="lineStyle">▍</span><span>日志信息</span>
    ...
    <el-col :span="8" >
      <el-card>
        <div slot="header" class="clearfix">
          <span class="lineStyle">▍</span><span>危险因素因果关系知识图谱快捷入口</span>
        </div>
    ...
    <el-col :span="8" >
      <el-card>
        ...
        <!-- <TaskCountChart :taskData="formattedChartData" /> -->
    ...
    <div class="bottomStatistic">
      <el-card class="bottom_statistic_card">
        <div slot="header" class="clearfix">
          <span class="lineStyle">▍</span><span>各病种正负样本占比</span>
        </div>
        <div id="chartBox">
...
export default {
  ...
  methods: {
    ...
    handleSetLineChartData(type) {
     ...
     getCausalRelationshipsNum(){
      getRequest(`admin/index/getKnowledgeGraphs/${this.user.id}`).then((response => {
        this.causalRelationshipsNum = response.data;
      }))
    },

    getDataNum(){
      getRequest(`admin/index/getTables/${this.user.id}`).then((response => {
        this.dataNum = response.data;
      }))
    },

    getMiningNum(){
      getRequest(`admin/index/getMinings/${this.user.id}`).then((response => {
    ...
    async getAll() {
      let id=11
      getRequest(`admin/index/getKnowledgeGraphsByDisease/${this.user.id}`).then(
        (response) => {
          console.log(response.data,'因果关系！！！！')
          ...
          for (let key in response.data) {
              ...
              if(key == "糖尿病"){
                    lenDiabetes=response.data[key]
                  }
              else if(key == "肺癌"){
                lenLung=response.data[key]
              } else if(key == "前列腺癌"){
                lenProstate=response.data[key]
              } else if(key == "高血压"){
                lenHypertension=response.data[key]
              } else if(key == "结直肠癌"){
                lenColorectal=response.data[key]
              }
  }
}
          lenRelationships=lenDiabetes+lenProstate+lenHypertension+lenColorectal
      ...
      await getRequest(`admin/index/getTablesByDisease/${this.user.id}`).then(
        (response) => {
          var lenDiabetes = 0
          var lenLung = 0
          var lenProstate = 0
          var lenColorectal = 0
          var lenHypertension = 0
          console.log(response.data,'各种数据集')
          for (let key in response.data) {
  if (response.data.hasOwnProperty(key)) {
    console.log(key, response.data[key]);
          if(key == "糖尿病"){
  ...
  this.dataChart.diabetes = lenDiabetes
          ...
          // this.dataChart.diabetes = 4
          // this.dataChart.lung = 7
          // this.dataChart.prostate = 4
          // this.dataChart.colorectal = 8
          // this.dataChart.hypertension = 12
