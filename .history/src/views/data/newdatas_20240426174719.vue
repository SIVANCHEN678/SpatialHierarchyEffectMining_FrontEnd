...
//提交对字段的管理
    ...
    toXLSX() {
      ...
      getRequest(`/api/getInfoByTableName/${this.showDataForm.tableName}`).then(
          (res) => {
            const header = this.selectedFields;
            const data = res.data;
            ...
            // 设置下载属性
            link.href = url;
            link.download = this.showDataForm.tableName + ".xlsx";
    ...
    upRequest(data) {
      ...
      console.log("数据类型!!!!!", this.dialogForm.region)
    ...
    addTable() {
      // 创建表
      this.diseaseName = this.dialogForm.dataDisease;
      console.log("YYYYYY", this.diseaseName)
