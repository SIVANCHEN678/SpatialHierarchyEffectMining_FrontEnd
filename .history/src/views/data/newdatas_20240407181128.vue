...
import { datatypes } from "@/components/tab/datatype.js";import { resetForm, debounce } from "@/components/mixins/mixin.js";
...
export default {
  ...
  methods: {
    ...
    submitTable() {
      ...
      this.$router.push('/data/newdatas')
    },
...
console.log('上传的数据',payload)
  this.$axios(options).then((res) => { // 返回表头信息
    ...
    if (res.code == "200") {
      ...
      this.featuresNum = res.data.length - 1;
...
,
    ...
    featureSelect(){
      this.loading = true;
     // this.dialogFormVisible1 = true;
     console.log(111,this.dialogForm.tableName)
      getRequest(`/admin/table/getHeader/${this.dialogForm.tableName}`).then(
        (response) => {
