...
export default {
  ...
  computed: {
  // 计算属性，用于计算 icon="el-icon-delete" 的个数
  deleteIconCount() {
    // 使用递归函数遍历树形结构的数据，并累加 delete icon 的数量
    const countDeleteIcons = (nodes) => {
      let count = 0;
      for (const node of nodes) {
        if (node.icon === 'el-icon-delete') {
          count++;
        }
        // 递归遍历子节点
        if (node.children && node.children.length > 0) {
          count += countDeleteIcons(node.children);
        }
      }
      return count;
    };
    // 初始遍历根节点
    return countDeleteIcons(this.treeData);
  }
},
  data() {
    return {
