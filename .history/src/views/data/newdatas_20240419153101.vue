        ...
        <el-card class="right_table_topCard">
          ...
          <div class="describe_content">
            ...
            <p style="margin-top:0.5%">
              <i class="el-icon-user"></i>数据集: <span>{{showDataForm.tableName}}  </span>
...
export default {
  ...
  data() {
    return {
      ...
      deleteIconCount: 5, // 用于存储 icon="el-icon-delete" 的个数
      sortKey: '', // 当前排序列的键名
...
,
    ...
    addTable(){ // 创建表
        ...
        if(this.addDataForm.dataName==''){
          this.$message.info('填写数据名称')
          return
        }
        if(this.addDataForm.dataType==null){
          this.$message.info('选择数据类型')
          return
        }

       let  path=this.showFeatureDataForm.classPath
       console.log(path,'路径')
       const pathParts = path.split('/');
    
    // 获取数组中最后一个元素，即疾病名字
    const diseaseName1 = pathParts[pathParts.length - 1];
console.log((JSON.parse(localStorage.getItem("user"))).name,'ID')
// 

      this.diseaseName = this.addDataForm.dataName//这是自己写的数据集的名字
      this.addDataForm.userId=(JSON.parse(localStorage.getItem("user"))).name

      this.addDataForm.diseaseName=diseaseName1
