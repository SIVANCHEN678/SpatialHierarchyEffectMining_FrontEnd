<template>
  <div>
    <div class="left_tree">
      <div slot="header" class="clearfix" style=" margin-left: 30%;font-size: 20px;">
        ...
        <p style="margin-left: 10%;">总共有{{totalPrimaryDiseases }}个一级病种,{{totalDatasets }}个数据集</p>
 ...
 <el-dialog
    ...
    <el-table-column v-for="(value, key) in tableData[0]" :key="key" :prop="key" :label="key" :min-width="100" width="auto" :fit="true" :sortable="true" @sort-change="handleSortChange(key)">
      <template slot-scope="{ row }">
        <div class="truncate-text">{{ row[key] }}</div>
      </template>
      <!-- 自定义表头 -->
      <template slot="header" slot-scope="scope">
        <div @click="handleSortChange(key)" style="cursor: pointer;">
          {{ key }}
          <i v-if="sortKey === key && sortOrder === 'ascending'" class="el-icon-arrow-up"></i>
          <i v-else-if="sortKey === key && sortOrder === 'descending'" class="el-icon-arrow-down"></i>
        </div>
      </template>
    </el-table-column>
  </el-table>
        ...
        <!-- <el-table :data="tableData" border style="width: 100%" class="custom-table">
    <el-table-column v-for="(value, key) in tableData[0]" :key="key" :prop="key" :label="key" :min-width="100" width="auto" :fit="true">
      <template slot-scope="{ row }">
        <div class="truncate-text">{{ row[key] }}</div>
      </template>
    </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      totalPrimaryDiseases: 10, // 假设一级病种的总数为10
      totalDatasets: 5 ,// 假设数据集的总数为5
      showFeatureDataForm: {
          tableName:'',
          createUser:'',
          createTime:'',
