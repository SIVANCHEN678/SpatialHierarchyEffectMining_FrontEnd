<template>
  <div>
    <div class="left_tree">
      <div slot="header" class="clearfix" style=" margin-left: 5px;">
          <span class="lineStyle">▍</span><span>数据纳排</span>
...
,
    ...
    changeData(data){
      ...
      // 展开/收缩当前节点的子节点
      this.$refs.tree.setCheckedKeys([node.key], true); // 若使用复选框功能则取消选中其他节点
      this.$refs.tree.setCurrentKey(node.key); // 高亮当前节点
      this.$refs.tree.toggleExpand(node.key); // 展开或收起当前节点

      // 如果还需要自动展开/收缩父节点，可以遍历父节点并展开
      const parentKeys = [];
      let parentNode = node.parent;
      while (parentNode && parentNode.data) {
        parentKeys.push(parentNode.key);
        parentNode = parentNode.parent;
      }
      
      // 从根节点开始逐级展开父节点
      parentKeys.reverse().forEach(key => {
        this.$refs.tree.expandNode(this.$refs.tree.getNode(key));
      });
    },
    // 其他方法...
