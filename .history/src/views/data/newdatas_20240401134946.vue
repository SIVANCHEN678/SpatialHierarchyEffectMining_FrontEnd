    ...
    <div class="right_table">
        <el-card class="right_table_topCard">
          ...
          <div class="tableData">
            ...
            <el-button type="primary" @click="showColumnSelection=true">导出数据</el-button>

            <el-dialog :title="'选择导出特征'"  :visible.sync="showColumnSelection">
            <el-checkbox :indeterminate="isIndeterminate" @change="handleCheckAllChange" label="全选">
              全选
            </el-checkbox>
            <el-checkbox-group v-model="selectedColumns">
              <el-checkbox v-for="item in allColumns" :label="item.uniqueKey" :key="item.uniqueKey">
                {{ idToItemMap[item.uniqueKey] }}
              </el-checkbox>
            </el-checkbox-group>
...
export default {
  ...
  methods: {
    handleCheckAllChange(val) {
      this.selectedColumns = val ? this.allColumns.map(column => column.key) : [];
      this.isIndeterminate = false;
    },
    handleCheckChange() {
      this.isIndeterminate = !!this.selectedColumns.length && !this.selectedColumns.length === this.allColumns.length;
    },
    show1(){
  ...
  try {
    ...
    FileSaver.saveAs(blob, 'selected_data.csv');
  } catch (err) {
    ...
    upRequest(data) {
      ...
      payload.append("user", (JSON.parse(localStorage.getItem("user"))).name);
      payload.append("uid", (JSON.parse(localStorage.getItem("user"))).id);
      payload.append("parentId", this.nodeData.id);
      payload.append("parentType", this.nodeData.path);
      console.log('name',(JSON.parse(localStorage.getItem("user"))).name)
    ...
    async getTableData(tableId, tableName) {
      try {
        ...
        if (Array.isArray(this.tableData) && this.tableData.length > 0) {
          ...
          console.log("数据长度" + this.tableData.length);
    ...
    mounted(){
      this.idToItemMap = this.allColumns.reduce((acc, item) => {
    acc[item.uniqueKey] = item;
    return acc;
  }, {});
    }
};
