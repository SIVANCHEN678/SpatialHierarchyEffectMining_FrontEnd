...
created() {
  setTimeout(() => {
      ...
      this.form2.miningType =this.miningType;
...
handleCheckAllChange(val) {
    ...
    getAllRelationships(){//关系挖掘   要从任务表中传过来from
      ...
      this.form.miningType = '';
      // 然后你可以直接给 this.form 添加新的属性
        this.form.newProperty = 0;
...
// 例如，如果你想添加一个名为 additionalInfo 的属性
    ...
    getAllRelationships2(){//关系挖掘   要从任务表中传过来from
      ...
      postRequest("api/mining/runing",this.form2).then(
        
        response => {
          const options = response.data

          console.log("因果关系",options)
          this.causalOptions1 = options
    ...
    diff1(){//关系挖掘   要从任务表中传过来from
      ...
      postRequest(`/api/mining/getDifferent?firstMining={this.causalOptions}&?secondMining={this.causalOptions1}`,{
