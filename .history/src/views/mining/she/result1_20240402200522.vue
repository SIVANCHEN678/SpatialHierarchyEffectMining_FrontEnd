    ...
    <el-row :gutter="40" class="panel-group">
      ...
      <div class="zhanshi"><el-col :span="12">
        <el-card style="margin-right: 20px ;height: 600px; margin-top: 50px;">
                ...
                <div v-if="!causalOptions" class="loading-animation">加载中...</div>
                <div v-else>
  ...
  data() {
    return {
      ...
      causalRelationshipsForm: {
        ...
        qrcodeSrc: require('../../../assets/book.png'), // 图片初始为空
...
created() {
  ...
  methods: {
    ...
    async  getImage1(){
      ...
      getRequest("admin/mining/admin/mining/getI").then(
    response => {
        // 检查response是否是有效的Base64字符串，如果不是，打印错误信息以调试
        if (typeof response === 'string') {
          this.qrcodeSrc = "data:image/png;base64," + base64ImageData.trim(); // 注意trim()用于去除可能存在的多余空白字符
            console.log("已传递");
        } else {
