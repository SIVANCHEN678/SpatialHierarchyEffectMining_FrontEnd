  ...
  <div v-else>
    ...
    <el-card style="margin-right: 20px ;height: 600px; margin-top: 50px;">
            ...
            <el-select v-model="choice" placeholder="请选择相应的空间层次" @change="getImage" size="medium" style="margin-left: 200px;">
              ...
              <div >
            <img :src="qrcodeSrc" alt="image" width="600" height="600" style="margin-top: -50px;">  
            </div>
...
created() {
  ...
  methods: {
    exportToCSV() {
      // 将表格数据转换为适合 CSV 格式的二维数组
      const tableDataForCSV = this.patientTable.map(row => Object.values(row));

      // 创建工作簿
      const workBook = XLSX.utils.book_new();

      // 创建工作表并将数据放入
      const workSheet = XLSX.utils.aoa_to_sheet(tableDataForCSV);
      XLSX.utils.book_append_sheet(workBook, workSheet, 'Patient Data');

      // 将工作簿转换为 Blob 对象
      const blob = XLSX.write(workBook, { bookType: 'csv', type: 'binary' });

      // 下载文件
      saveAs(new Blob([this.s2ab(blob)], { type: "application/octet-stream" }), "patient_data.csv");
    },
    s2ab(s) {
      // 将字符串转换为 ArrayBuffer
    ...
    exportToCSV() {
      // 将表格数据转换为CSV格式
      let csvData = [];
      const headers = Object.keys(this.patientTable[0]);
      ...
      this.patientTable.forEach(row => {
        const data = headers.map(fieldName => `"${row[fieldName]}"`).join(',');
        csvData.push(data);
      });

      // 创建Blob对象并下载
      const csvBlob = new Blob([csvData.join('\n')], { type: 'text/csv;charset=utf-8;' });
      FileSaver.saveAs(csvBlob, 'patient_data.csv');
    },
