    ...
    <el-row :gutter="40" class="panel-group" >
     ...
     <div class="zhanshi"  >
      ...
      <div slot="header" class="clearfix">
        ...
        <div class="taskInfoBox dataset" style="margin-left: -10px;">
          <span class="lineStyle">▍</span><span class="featureTitle"><strong>所用数据：</strong></span>
...
export default {
  ...
  //this.getImage1()
    ...
    console.log('这个form1',this.form.name)
    this.getlayer()
...
//获取每层的特征  选择那里的展示
  ...
  if (typeof window !== 'undefined') {
    ...
    async getAllRelationships(){//关系挖掘   要从任务表中传过来from
      console.log('表格', this.form.name)
...
// 例如，如果你想添加一个名为 additionalInfo 的属性
      ...
      const response = await postRequest("api/mining/runing1", this.form);
    ...
    async getAllRelationships2(){//关系挖掘   要从任务表中传过来from
      console.log('表格信息',this.form2)
      const response = await postRequest("api/mining/runing1", this.form2);
      const options = response.data;
    ...
    async diff1(){//关系挖掘   要从任务表中传过来from
      await this.getAllRelationships();
      await this.getAllRelationships2();
      console.log('第一个关系',this.causal1)
      console.log('第二个关系',this.causal2)
      postRequest('/api/mining/getDifferent?firstMining='+this.causal1+'&secondMining='+this.causal2,{}).then(
        response => {
