    ...
    <el-card style="margin-right: 20px ;height: 600px; margin-top: 50px;">
            ...
            <el-select v-model="choice" placeholder="请选择相应的空间层次" @change="getImage" size="medium" style="margin-left: 200px;">
                <el-option
                  ...
                  <div >   <div v-if="isLoading" class="loading-animation">加载中...</div>
    <img :src="qrcodeSrc" v-else-if="qrcodeSrc" alt="image" width="600" height="600" style="margin-top: -50px;">
...
export default {
    ...
    return {
      ...
      causalRelationshipsForm: {
        ...
        isLoading: true, // 设置加载状态为true
         defaultQrcodeSrc: require('@/assets/she/she.png'), // 替换为实际的默认图片路径
  ...
  mounted() {
    if (!this.qrcodeSrc) {
    this.defaultQrcodeSrc = require('../../../assets/book.png');
  }
...
// },
async getImage() {
  getRequest.get(`admin/mining/getImage/${this.form.tableName}${this.choice}`, { responseType: 'arraybuffer' })
      .then(response => {
        // 将ArrayBuffer转换为Base64编码字符串
        const base64Image = this.arrayBufferToBase64(response.data);
        this.qrcodeSrc = `data:image/png;base64,${base64Image}`;
        this.isLoading = false; // 加载完成，设置加载状态为false
      })
      .catch(error => {
        console.error('Error fetching image:', error);
        this.isLoading = false; // 加载失败，设置加载状态为false
      });
