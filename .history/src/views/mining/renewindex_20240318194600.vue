      ...
      <div v-if="active == 3">
          ...
          <div id="mainform">
            <div id="ll">
              ...
              <el-radio-group v-model="selectedPosition" style="margin-bottom: 30px">
                <el-radio-button v-for="(option, index) in positions" :key="index" :value="option.value">{{ option.label }}</el-radio-button>
              </el-radio-group>

              <el-tabs :tab-position="selectedPosition" style="height: 200px" class="demo-tabs">
                <el-tab-pane v-for="(tab, index) in tabs" :key="index" :label="tab.label">{{ tab.content }}</el-tab-pane>
              </el-tabs>
...

{
        ...
        this.positions = data.positions; // 假设从后端获取的数据中有一个名为 positions 的数组，包含了选项的值和标签信息
        this.selectedPosition = this.positions[0].value; // 默认选中第一个选项
      } 
    },
    async fetchTabs() {
      try {
        // 模拟异步获取选项卡数据的过程，实际项目中应该使用真实的接口请求
        const response = await fetch('your_tabs_api_endpoint');
        const data = await response.json();
        this.tabs = data.tabs; // 假设从后端获取的数据中有一个名为 tabs 的数组，包含了选项卡的标签和内容信息
      } catch (error) {
