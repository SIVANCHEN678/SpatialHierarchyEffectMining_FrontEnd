...
<div v-if="active==1" >
  ...
  <div id="center">
    ...
    </h5><div class="checkbox-container">
        <el-tooltip effect="light" content="全选" placement="top">
          <el-checkbox class="radio" :indeterminate="isIndeterminate" v-model="checkAllDemoOptions" @change="handleCheckAllChange" border>
            全选
          </el-checkbox>
        </el-tooltip>
      </div>
...
import { validAlphabets } from '@/utils/validate';
  export default {
      ...
      if (val) {
        this.checkedDemoOptions = this.demoOptions.filter(item => item.columnisR).map(item => item.columnName);
      } else {
        this.checkedDemoOptions = [];
      }
      console.log(this.checkAllDemoOptions)
    },
