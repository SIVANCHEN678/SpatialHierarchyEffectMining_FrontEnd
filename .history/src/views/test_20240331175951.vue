<template>
  ...
  <div ref="ct" style="width: 100% ; height: 800px;" class="chinaMapBox">
    <h1 style="margin-left: 700px; color: #fff;">选择展示因果关系的地区</h1>
    <div ref="charts" :style="{ height: chartsHeight }" />
    <div v-if="currentClickedCity" class="zi" style="margin-left: 300px;">
    当前点击的地区：{{ currentClickedCity }}
  </div>
  <el-button @click="next()" type="primary" style="margin-left: 800px;">确认</el-button>
  <el-button @click="fanhui()" type="primary" style="margin-left: 800px;">返回</el-button>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    this.lastCity = this.citylist[this.citylist.length - 2];
    const name = Object.keys(this.lastCity)[0];
    const [graphName, graphJson] = getMap(name);
    this.citylist.pop();
    option.geo.map = graphName;
    echarts.registerMap(graphName, graphJson);
    this.charts.setOption(option, true);
    }
