...
export default {
  ...
  methods:{
    getUserDetail(){
      getRequest(`/admin/user/getmessage/${this.uid}`).then(res => {
        if(res.code === 200){
          console.log(res)
          let data = res.data
          ...
          this.form.answer_1 = data.answer1.split(":")[1]
          this.form.answer_2 = data.answer2.split(":")[1]
          this.form.answer_3 = data.answer3.split(":")[1]
          this.form.uploadSize = data.uploadSize
          let test =  `${this.form.question1}:${this.form.answer_1 }`;
          console.log(test)
          console.log('问题1',this.form.question1)
        }
      })
    },
    checkRepetition(){
        getRequest(`/admin/user/checkRepetition/${this.form.username}`).then(res=>{
          if(res.code === 200){
            if(res.msg === "用户名已存在"){
    ...
    submitForm(){
      ...
      form.answer1 =`${this.form.question1}:${this.form.answer_1 }`
      form.answer2 = `${this.form.question2}:${this.form.answer_2 }`
      form.answer3 = `${this.form.question3}:${this.form.answer_3 }`
      form.uid=sessionStorage.getItem("uid")
      console.log('修改信息',form)
      postRequest('/admin/user/updateUser',form).then(res => {
