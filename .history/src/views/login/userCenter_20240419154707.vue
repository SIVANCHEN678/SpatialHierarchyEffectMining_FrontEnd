...
export default {
  ...
  methods:{
    getUserDetail(){
      console.log(this.form.uid,'this.uid')
      getRequest(`admin/user/getmessage/${this.form.uid}`).then(res => {
        if(res.code === 200){
          let data = res.data
    ...
    submitForm(){
      ...
      postRequest('/admin/user/updateUser',this.form).then(res => {
        console.log(res.data,'修改的信息')
        if(res.code === 200){
