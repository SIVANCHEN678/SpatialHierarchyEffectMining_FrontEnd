...
this.dis=response.data
      ...
      getList(page = 1) { //添加当前页参数
            this.current = page
            let id=11
            postRequest(`/admin/knowledgeGraph/findKnowledgeGraph/${this.current}/${this.limit}/${id}`,this.queryParam)
      ...
      postRequest(`admin/knowledgeGraph/findKnowledgeGraph/${this.current}/${this.limit}/${this.user.id}`,this.queryParam)
        .then(response => { //请求成功response是接口返回数据
