<template>
  <div class="app-container">
    ...
    <div class="left_tree">
      ...
      <div v-for="(item,index) in list1" :key="index"  style="font-size: 20px; ">{{ item.name }}</div>
      <!-- <el-tree
        ref="tree"
        :data="treeData"
        :show-checkbox="false"
...
<div class="abc">
  ...
  <div class="horizontal-line"></div>
    ...
    <el-row :gutter="20">
      <el-col :span="6" v-for="(item,index) in list1" :key="index" style="margin-bottom: 10px;margin-left: 35px; width: 350px;height: 130px;">
        <el-card body-style="padding: 10px; height: 100px;   ">
          <div  class="clearfix" >
            <!-- <span><strong>任务</strong>{{ item.id }}</span> -->
           
          </div>
          <div><strong>知识图谱关系：</strong>{{  }}</div>
  ...
  export default {
    ...
    methods: {
      getdis(){
        getCategory("/admin/relationship/getAllDisease").then((response)=>{
       //this.treeData = response.data;
       this.list1=response.data,
       console.log(this.dis,'树')
      })
      },
      getCatgory(){
      ...
      //           })
      //   },
        getList1(page=1) { //添加当前页参数
          this.current = page
          getRequest(`/admin/relationship/findTables/${this.current}/${this.limit}/${this.searchObj}`)
                .then(response => { //请求成功response是接口返回数据
                //返回集合赋值list
               // this.list1 = response.data
               this.total = response.data.total
