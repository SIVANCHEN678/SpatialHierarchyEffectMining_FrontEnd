...
<script>
import { data } from '../views/knowledgeGraph/data'
  import{
      expendNodes,
  ...
  export default {
      ...
      async mounted() {
    // 初始化数据
    const initialData = data[0]; // 这里以data数组的第一个元素作为初始展开的数据
    await this.formatData([initialData], true); // 第一个参数是初始数据，第二个参数设为true代表重置图表
},
      beforeDestroy(){
