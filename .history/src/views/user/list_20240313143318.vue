    ...
    <el-row :gutter="20">
      <el-col :span="6" v-for="(item, index) in tableData" :key="index">
        <el-card class="box-card" @click.native="handleCardClick(item)">
          ...
          <div><span>创建时间：</span>{{ item.registerTime }}</div>
          <div><span>用户角色：</span>{{ item.role }}</div>
          <div><span>用户性别：</span>{{ item.gender }}</div>
          <el-button type="danger" style="margin-top: 20px;margin-left: 50px;" @click.stop="deleteTable(item)">删除</el-button>
        </el-card>
      </el-col>
    </el-row>
