<template>
  <div class="content" style="display: flex; width: 100%; height: 100vh;">
    <div class="left_tree" style="width: 20%; /* 根据需要调整宽度 */">
      <div class="tipInfo">
        <h3>可选数据</h3>
    ...
    <el-dialog
      ...
      >
          ...
          >
              ...
              <el-form-item label="数据是否包含地区信息" prop="datatype">
              <el-select
          ...
          <div slot="footer" class="dialog-footer">
              ...
              >取消</el-button
              >
              <el-button @click="resetForm1('dialogFormRef')">重置</el-button>
    ...
    <el-card class="right_table_topCard" :body-style="{ height: '850px', width: '100%' }">
      ...
      <div class="describe_content">
        <p style="margin-top: 0.5%; width: 100%">
          ...
          <el-button type="success" size="mini" class="change_btn" v-if="tableData.length && nodeData.uid === loginUserID && nodeData.status == '0'" @click="TableshareUserSelectDialog = true">转为共享</el-button>
          <el-button type="success" size="mini" class="change_btn" v-if="tableData.length && nodeData.uid === loginUserID && nodeData.status == '1'" @click="changeToPrivate()">转为私有</el-button>
          <el-button type="warning" size="mini" class="change_status_btn" v-if="tableData.length && nodeData.uid === loginUserID && nodeData.status == '1'" @click="changeShareStatus()">更改共享人员</el-button>
          <el-button type="success" @click="csvDialogVisible = true" v-if="(tableData.length && nodeData.status == '0' && nodeData.isUpload=='1') || downloadStatus == '2'" class="csv_btn" size="mini">允许下载</el-button>
          <el-button type="primary" @click="applyDownload()" v-else-if="tableData.length && downloadStatus == '0'" class="csv_btn" size="mini">申请下载</el-button>
          <el-button type="warning" @click="waitingCheck()" v-else-if="tableData.length && downloadStatus == '1'" class="csv_btn" size="mini">等待审核</el-button>
        </p>
      </div>
      <!-- 显示表数据 -->
...
import { region } from "@/components/tab/region.js";
let id = 1000;
...
export default {
  ...
  methods: {
    ...
    next(){
      ...
      console.log(this.selectTableName,'表名')
      console.log(this.selectTableName)
            if(this.selectTableName === null){
              this.$message.error("请先选择数据");
            }else{



              getRequest(`/api/region/checkRegion/?tableName=${this.selectTableName}`,).then(
              (res) => {
                console.log(res)
             
                  
                    this.m_changeTaskInfo({ m_tableId: this.selectTableName})
                    this.m_changeTaskInfo({ m_dataset: res.data.tableName });//直接设置 表名
                    this.m_changeTaskInfo({ m_have: res.code });
                    this.m_changeTaskInfo({ m_disease: res.data.diseaseName});//设置病种  获取的 疾病名字    
                    this.m_changeTaskInfo({ m_dataType: res.data.dataType });//直接设置
                    this.m_changeStep(3);
...
for(let i=0; i < FeaturesLen; i++){
  ...
  payload.append("data_type", this.dialogForm.datatype);
  ...
  this.$axios(options).then((res) => { // 返回表头信息
    ...
    changeData(data) {
      if (data.isLeafs == 1) {
        ...
        // 将当前点击的数据id保存下来
        this.selectTableName = data.label;
      }
    },
    append(isLeaf) {
