<template>
  <div v-loading="loading" element-loading-text="拼命运算中">
    <div class="titleBox1"> <p style="margin-top: -1%;">
      <strong>可选择的算法 </strong></p></div>
    <el-container> 
     
      <el-tabs id="modelList"  v-model="model" tab-position="left" @tab-click="onTabClick">
          ...
          <div class="introBox">
           ...
           <div class="paramBox">
            ...
            >
              <el-form-item prop="K_OR">
                <template slot="label">
                  ...
                  <el-popover placement="right"  trigger="click">
                    <div>
                      空间层次效应挖掘对应选择了三层，临床表征指标、社会环境指标、生活行为习惯指标，临床-疾病危险因素则对应选择少于三层的特征信息
                    </div>
                    <el-icon
                      slot="reference"
              ...
              <el-input  v-model="selectedtask" :disabled="true"  style="margin-top: 2%;">
                          <!-- <el-option
                                  label="空间层次效应挖掘"
                                  value="she_mining"
                          ></el-option>
              ...
              <el-form-item prop="K_and_SP">
                <template slot="label">
                  ...
                  <el-popover placement="right" trigger="hover">
                    <div>
                      点击选择上方的算法根据数据的类型选择合适的算法参与运算，对于fges算法运算速度较快，算法只能处理数值类型的数据
                    </div>
                    <el-icon
                      slot="reference"
                      class="el-icon-warning-outline"
                ...
                <el-input v-model="tab.algorithmName" style="margin-top: 5px;"  :disabled="true">
              ...
              <el-form-item prop="K_and_PC">
                <template slot="label">
                  ...
                  <el-popover placement="right" trigger="hover">
                    <div>
                      设置结点最大度数包括出度和入度，即在搜索过程中允许的最大边数量，默认为3，请根据任务选择合适的度
                    </div>
                    <el-icon
                      slot="reference"
                      class="el-icon-warning-outline"
              ...
              <el-form-item prop="K_and_PC">
                <template slot="label">
                  ...
                  <el-popover placement="right" trigger="hover">
                    <div>
                      在搜索因果图边缘时，指定用于评价和选择因果图模型的评分方法，分值越高结果越准确
                    </div>
                    <el-icon
                      slot="reference"
                      class="el-icon-warning-outline"
...
import { from } from 'json2csv/lib/JSON2CSVTransform';
export default {
...
mounted(){
  ...
  if (!this.all.m_clinic || !this.all.m_living || !this.all.m_social) {
    this.selectedtask = "临床表征-疾病危险因素因果关系挖掘";
    // Here you can use the variable `s` as needed.
}else{
  this.selectedtask="空间层次效应挖掘"
}
 ...
 // this.search1();
  ...
  created() {
    ...
    console.log("BBBBBBBB",this.$store.state)
    this.init();
    this.searchAll(); // 在组件挂载后调用获取标签页数据的方法
  },
...
searchAll () {  //搜寻所有算法
  ...
  getRequest(`/admin/algorithm/getAll/${this.all.m_dataType}`).then(
    (response) => {
      console.log('1', response)
      ...
      // 添加这一行，尝试找到 'pc' 算法并激活对应的选项卡
        ...
        this.algorithm= response[0].algorithmName
 ...
 //提交整个任务
      submitForm() {
       console.log(this.algorithm,'算法名字')
  console.log(sessionStorage.getItem("userid"),'iid')
      if (this.selectedss  && this.selectedmax &&this.selectedtask) {
      // 执行提交表单的操作
        console.log("提交表单");}
      // } else {
      // // 显示错误提示
      ...
      this.form.dataType=this.all.m_dataType;
      this.form.scoreId=this.selectedss;
      this.form.maxDegree=this.selectedmax;
      this.form.algorithmId=this.algorithm ;
      ...
      this.form.miningType=this.miningType;
      ...
      postRequest("api/mining/insert", this.form).then(
          response =>{
              console.log(response)
              if (response.code == "200") {
                  this.$message.success("任务创建成功")
                
                  this.$router.push({ path: '/sheResult' ,query: { form: this.form} })
