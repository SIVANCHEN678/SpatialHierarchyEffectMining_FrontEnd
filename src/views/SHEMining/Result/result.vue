...
//保存第一层
getRequest(`/admin/knowledgeGraph/saveDisease2Factor/${context}/${diseaseName}/${this.user.id}`).then(
            response =>{
...
//保存第二层
postRequest(`/admin/knowledgeGraph/saveFactor2Factor/${this.user.id}`, this.requstForm).then(
            response =>{
              console.log('第二层',response.data)
              if (response.code == "200") {
               this.$message.success("保存成功")
              
                this.$router.push({ path: '/knowledgeGraph' })
