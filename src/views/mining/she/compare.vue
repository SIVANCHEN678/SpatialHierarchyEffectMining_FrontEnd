<template>
 ...
 <div v-else>
  ...
  <el-button class="buttonGroup" type="primary" @click="exportToPDF"  >导出结果</el-button>
   <el-row :gutter="40" class="panel-group" >
    ...
    <div class="zhanshi"  >
            ...
            <el-col :span="12">
            <el-card style="margin-right: 2% ;height: 180px;width:95%; margin-top: 30px;margin-left: 1%;">
     <div slot="header" class="clearfix">
             <span class="lineStyle">▍</span><span class="featureTitle"><strong>对比任务1疾病危险因素因果关系挖掘信息</strong></span>
       ...
       <div class="taskInfoBox dataset" style="margin-left: -20px;">
     ...
     <div slot="header" class="clearfix">
             <span class="lineStyle">▍</span><span class="featureTitle"><strong>对比任务2疾病危险因素因果关系挖掘信息</strong></span>
             <!-- <el-button type="primary" @click="back()" style="margin-left: 60%;">返回</el-button>
             <el-button type="primary" @click="exportToPDF()" >导出结果</el-button> -->
       <!-- <el-button type="primary" @click="totask">查看任务</el-button> -->
       </div> 
   ...
   <el-row :gutter="40" class="panel-group" >
     <div class="zhanshi"  >
       <el-col :span="12">
         <el-card  style="margin-right: 30px;margin-top: 50px;margin-left:10px; height: 650px;"        
             >  <div slot="header" class="clearfix">
                <span class="lineStyle">▍</span><span class="featureTitle"><strong>对比任务1因果关系展示</strong></span>
                      ...
                      <el-dialog title="请选择详细查看的关系" :visible.sync="csvDialogVisible" width="60%" :before-close="handleCloseCSV"> 
                       ...
                       <el-row :gutter="4">
                           <h5 class="text" style="font-size: 20px;margin-bottom: 10px; font-weight: bold;">
                             <span class="lineStyle">▍</span>临床表征指标  <el-checkbox  :indeterminate="isIndeterminate3" v-model="selectAlll"  /> 全选
                           </h5>
                           <el-col :span="24">

                                         <el-checkbox-group v-model="selectedclinical" >
         ...
         <h5 class="text" style="font-size: 20px; font-weight: bold;"> <span class="lineStyle">▍</span>社会环境指标
           <el-checkbox :indeterminate="isIndeterminate1" v-model="selectAlls"  /> 全选
         </h5>
          
       
         <el-col :span="24">

           <el-checkbox-group v-model="selectedsociol" >
         ...
         <el-row :gutter="4">
           <h5 class="text" style="font-size: 20px; font-weight: bold;"> <span class="lineStyle">▍</span>生活行为习惯指标
             <el-checkbox v-model="selectAllss" :indeterminate="isIndeterminate2"  /> 全选
           </h5>
           
           <el-col :span="24">
                            
             <el-checkbox-group v-model="selectedliving" >
       ...
       <el-col :span="12">
         <el-card  style="margin-right: 30px;margin-top: 50px;margin-left:10px; height: 650px;"        
             >  <div slot="header" class="clearfix">
                <span class="lineStyle">▍</span><span class="featureTitle"><strong>对比任务2因果关系展示</strong></span>
     ...
     <el-button v-if="showbutton==true" class="buttonGroup" type="success" @click="dif() ">查看不同因果关系</el-button>
   <el-row :gutter="40" class="panel-group">
     
 <div class="zhanshi" v-if="showbutton==false">
      
  <el-col :span="12">
     <el-card style="margin-right: 2%;margin-top: 10px;margin-left:2%; height: 350px;">
         <div slot="header" class="clearfix">
           <span class="lineStyle">▍</span><span class="featureTitle"><strong>对比任务1包含因果关系</strong></span>
         </div>
         ...
         <div id="mainform" >
           <!-- <el-checkbox :indeterminate="isIndeterminate" v-model="checkAll" @change="handleCheckAllChange" >全选</el-checkbox> -->
                    ...
                    <div class="row">
            <span v-for="(relation, index) in diff1" :key="index">{{ relation }} </span>
        </div>
   ...
   <el-col :span="12">
     <el-card style="margin-right: 2%;margin-top: 10px;margin-left:2%; height: 350px;">
         <div slot="header" class="clearfix">
           <span class="lineStyle">▍</span><span class="featureTitle"><strong>对比任务2包含因果关系</strong></span>
         </div>
         ...
         <div id="mainform" >
           <!-- <el-checkbox :indeterminate="isIndeterminate" v-model="checkAll" @change="handleCheckAllChange" >全选</el-checkbox> -->
                    ...
                    <div class="row">
            <span v-for="(relation, index) in diff2" :key="index">{{ relation }} </span>
        </div>
...
created() {
 ...
 methods: {
  ...
  // 使用Promise.all来确保两个请求都完成后再继续
  Promise.all([
    this.form2.miningType = 0,
    this.form.miningType = 0,
    postRequest("api/mining/runing1", this.form2).then(response => {
      this.ca2 = response.data;
      console.log(this.ca2);
    }),
    postRequest("api/mining/runing1", this.form).then(response => {
      this.ca1 = response.data;
    })
  ]).then(() => { // 当两个请求都完成时执行这里的回调
    console.log('this.causal1', this.ca1);
    console.log('this.causal2', this.ca2);

    // 现在可以安全地使用ca1和ca2进行下一个请求
    postRequest('/api/mining/getDifferent?firstMining=' + this.ca1 + '&secondMining=' + this.ca2).then(
      response => {
        console.log(response.data, '不同关系');
        this.diff1 = response.data.diffFirst;
        this.diff2 = response.data.diffSecond;
        console.log("不同因果关系1", this.diff1);
         // 数据加载完成后强制更新页面
        // 数据加载完成后手动触发更新
    this.$nextTick(() => {
      this.$forceUpdate();
  ...
  // 移除了原先直接在函数末尾的console.log和getdif()调用，因为它们应该在异步操作完成后执行
},
...
html2canvas(element, { useCORS: true }).then((canvas) => {
    ...
    downloadLink.download = 'exported-image.png'; // 设置图片文件名
...
.row {
    display: flex;
    flex-wrap: wrap; /* 允许行内元素换行，如果内容超出一行的话 */
}

.row span {
    margin-right: 10%; /* 可以调整间距 */
}
</style>
