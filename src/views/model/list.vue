<template>
  ...
  export default {
    ...
    methods: {
      clearn(){
        this.queryParam.algorithmName=null,
        this.queryParam.algorithmType=null
        this.searchAll();
      },
      //未写
    //   getList(page = 1) { //添加当前页参数
    //   this.current = page;
    //   postRequest(`/admin/algorithm/findTables/${this.current}/${this.limit}`, this.queryParam)
    ...
    // },
        searchAll () {
          ...
          getRequest('/admin/algorithm/getAllAlgorithm').then(
                  (response)=>{
