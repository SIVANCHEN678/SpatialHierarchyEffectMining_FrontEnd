...
export default {
  ...
  methods: {
      getAllInform(){
          getRequest('/api/notice/queryNotices').then((res)=>{
    ...
    getInformTable(pageNum) {
      getRequest(
        "/api/notice/allNotices?current=" + pageNum + "&limit=" + 10
      ).then((res) => {
        console.log('获取数据',res)
        if (res) {
          const dataWithEditing = res.data.records.map((item) => ({
            ...item,
    ...
    saveChanges(row) {
      ...
      postRequest("api/notice/updateNotice", row).then((res) => {
        if (res.code == 200) {
    ...
    deleteInform(row) {
      postRequest("api/notice/delNotice", row).then((res) => {
        if (res.code == 200) {
    ...
    addInform() {
      ...
      postRequest("api/notice/insertNotice", params).then((res) => {
        if (res.code == 200) {
