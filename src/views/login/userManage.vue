...
export default {
  ...
  methods: {
      getUserAll(){
          getRequest('admin/user/querUser').then((res)=>{
              if(res){
                    this.tableData = res;
              }
          })
    ...
    getUserTable(pageNum) {
      getRequest("admin/user/allUser?pageNum=" + pageNum).then((res) => {
        if (res) {
    ...
    toggleEditing(row) {
      console.log("XXXXXX",row.editing)
      if (row.editing) {
    ...
    saveChanges(row) {
      ...
      postRequest("admin/user/updateStatus", params).then((res) => {
        if (res.code == 200) {
    ...
    deleteUser(row) {
      ...
      postRequest("admin/user/delUser", params).then((res) => {
        if (res.code == 200) {
          console.log("删除成功");
