  ...
  methods: {
    ...
    next() {
      if (this.active === 1) {
        this.$refs.forgetForm1.validate((valid) => {
          if (valid) {
            getRequest(
              "/admin/user/queryQuestions?username=" + this.forgetForm1.username
            ).then((res) => {
    ...
    customValidation() {
      if (this.active === 2) {
        ...
        postRequest("/admin/user/verify", params).then((res) => {
    ...
    checkUsernameExist(rule, value, callback) {
      ...
      getRequest("/admin/user/querUserNameExist?userName=" + value).then((res) => {
        console.log("res", res);
        if (res.code == 200) {
       ...
       if(this.active===3){
         this.$refs.forgetForm3.validate((valid) => {
           if(valid){
             ...
             postRequest('/admin/user/updatePwd',params).then((res)=>{
               console.log('res',res)
               if(res){
               if(res.code===200){
